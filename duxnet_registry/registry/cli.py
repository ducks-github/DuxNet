...
from duxnet.wallet.address_utils import validate_address

class NodeRegistryCLI:
    """Command-line interface for node registry management."""

    def __init__(self, persistence_path: Optional[str] = '/var/lib/duxnet/nodes.json', 
                 log_level: str = 'INFO', 
    ...
    def register_node(self, args):
        ...
        try:
            ...
            node = Node(
                ...
                duxnet_version=args.duxnet_version,
    ...
    def list_nodes(self, args):
        ...
        print(f"{'Node ID':<40} {'Wallet Address':<40} {'Hostname':<25} {'IP Address':<20} {'Reputation':<10} {'Healthy':<10}")
        print("-" * 145)
        for node in nodes:
            print(f"{node.id:<40} {node.wallet_address:<40} {node.hostname:<25} {node.ip_address:<20} {node.reputation_score:<10.2f} {node.health.is_healthy!s:<10}")
            ...
            def clean_dict(d):
                if isinstance(d, dict):
                    return {k: clean_dict(v) for k, v in d.items() 
                            if not (hasattr(v, '_mock_return_value') or 
                                    hasattr(v, '_mock_side_effect'))}
                return d
    ...
    def update_node_reputation(self, args):
        ...
        :param args: Parsed command-line arguments
        """
        result = self.registry.update_node_reputation(
            args.node_id, 
            args.task_success
        )
    ...
    def create_parser(self):
        ...
        parser = argparse.ArgumentParser(
            ...
            epilog="For more information, visit https://duxnet.org/node-registry"
        )
        ...
        register_parser.add_argument(
            '--ip-address', 
            required=True, 
            help='Valid IPv4 or IPv6 address of the node'
        )
        register_parser.add_argument(
            '--hostname', 
            help='Optional hostname for the node',
            default=''
        )
        register_parser.add_argument(
            '--os-version', 
        ...
        register_parser.add_argument(
            ...
            type=int, 
            default=0, 
            help='Number of CPU cores (non-negative integer)'
        )
        register_parser.add_argument(
            '--memory-gb', 
            type=float, 
            default=0.0, 
            help='Total memory in GB (non-negative float)'
        )
        register_parser.add_argument(
            '--storage-gb', 
            type=float, 
            default=0.0, 
            help='Total storage in GB (non-negative float)'
        )
        register_parser.add_argument(
            '--gpu-enabled', 
            action='store_true', 
            help='Flag to indicate GPU availability'
        )
        register_parser.add_argument(
            '--gpu-model', 
            help='Optional GPU model name'
        )
        ...
        deregister_parser.add_argument(
            'node_id', 
            help='Unique node ID to deregister'
        )
        ...
        list_parser.add_argument(
            '--min-reputation', 
            type=float, 
            help='Minimum reputation score to filter nodes'
        )
        list_parser.add_argument(
            '--only-healthy', 
            action='store_true', 
            help='Show only healthy nodes'
        )
        list_parser.add_argument(
            '--min-cpu', 
            type=int, 
            help='Minimum number of CPU cores'
        )
        list_parser.add_argument(
            '--min-memory', 
            type=float, 
        ...
        get_parser.add_argument(
            'node_id', 
            help='Node ID or wallet address to retrieve'
        )
        ...
        health_parser.add_argument(
            'node_id', 
            help='Node ID to update health metrics'
        )
        health_parser.add_argument(
            '--load-average', 
            type=float, 
        ...
        rep_parser.add_argument(
            'node_id', 
            help='Node ID to update reputation'
        )
        rep_parser.add_argument(
            '--task-success', 
            action='store_true', 
        ...
        parser.add_argument(
            '--log-file', 
            help='Path to the log file for persistent logging'
        )
