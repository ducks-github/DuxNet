...
from duxos_escrow.models import Escrow, EscrowStatus, CommunityFund, EscrowTransaction
...
class TestEscrowCoreIntegration:
    ...
    def test_escrow_creation_with_real_integration(self, escrow_manager, db_session):
        ...
        if REGISTRY_AVAILABLE:
            ...
            wallet_repo.get_wallet_by_id = Mock(side_effect=lambda x: payer_wallet if x == 1 else provider_wallet)
            ...
            async def test_concurrent_locks():
                ...
                for i in range(5):
                    task = asyncio.create_task(
                        asyncio.to_thread(
                            integration.lock_funds, 1, 10.0, f"escrow_{i}"
                        )
                    )
                    tasks.append(task)
    ...
    def test_audit_trail(self, escrow_manager, db_session):
        ...
        transactions = db_session.query(EscrowTransaction).filter(
            EscrowTransaction.escrow_id == escrow.id
        ).all()
        ...
        with patch('duxos_escrow.wallet_integration.FlopcoinWalletService', return_value=mock_wallet_service):
            ...
            async def concurrent_operations():
                ...
                for i in range(5):
                    task = asyncio.create_task(
                        asyncio.to_thread(
                            integration.transfer_funds, None, 2, 5.0, f"transfer_{i}"
                        )
                    )
                    tasks.append(task)
