...
async def quick_test():
    ...
    try:
        ...
        registry = IntegratedNodeRegistry(
            node_id="quick_test_node", db_session=db, p2p_listen_port=9338, p2p_broadcast_port=9339
        )
        ...
        if updated_node:
            print(
                f"✅ Updated node - Status: {updated_node['status']}, Reputation: {updated_node['reputation']}"
            )
