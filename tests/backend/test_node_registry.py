...
def sample_node():
    ...
    return Node(
        ...
        capabilities=NodeCapabilities(
            cpu_cores=4, memory_gb=16.0, storage_gb=500.0, gpu_enabled=True
        ),
    )
...
def test_register_duplicate_node(sample_node):
    ...
    second_node = Node(
        wallet_address="test_wallet_123", ip_address="192.168.1.101", hostname="test-node-2"
    )
...
def test_update_node_health(sample_node):
    ...
    # Update health
    result = registry.update_node_health(
        node_id, load_average=1.5, memory_usage=50.0, disk_usage=30.0
    )
    assert result is True
...
def test_list_nodes(sample_node):
    ...
    node2 = Node(
        ...
        capabilities=NodeCapabilities(cpu_cores=2, memory_gb=8.0, storage_gb=250.0),
    )
    ...
    min_caps = NodeCapabilities(cpu_cores=4, memory_gb=16.0, storage_gb=500.0)
