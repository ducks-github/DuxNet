...
from .models import Service, Review, Rating, ServiceUsage, SearchFilter, ServiceStatus, ServiceCategory
...
class StoreService:
    ...
    def register_service(self, service_data: Dict[str, Any], owner_id: str, owner_name: str) -> Service:
    ...
    def update_service(self, service_id: str, updates: Dict[str, Any], owner_id: str) -> Optional[Service]:
        ...
        allowed_updates = {
            "name", "description", "version", "tags", "documentation_url", 
            "repository_url", "price_per_call", "price_currency", "free_tier_calls",
            "rate_limit_per_hour", "input_schema", "output_schema", 
            "example_input", "example_output", "metadata"
        }
    ...
    def get_services_by_owner(self, owner_id: str, status: Optional[ServiceStatus] = None) -> List[Service]:
    ...
    def search_services(self, search_filter: SearchFilter) -> Tuple[List[Service], int]:
        ...
        if search_filter.sort_by == "rating":
            candidates.sort(key=lambda s: s.average_rating, reverse=(search_filter.sort_order == "desc"))
        elif search_filter.sort_by == "price":
            candidates.sort(key=lambda s: s.price_per_call, reverse=(search_filter.sort_order == "desc"))
        elif search_filter.sort_by == "date":
            candidates.sort(key=lambda s: s.published_at or s.created_at, reverse=(search_filter.sort_order == "desc"))
        else:  # relevance - use weighted score
            candidates.sort(key=lambda s: self._calculate_relevance_score(s, search_filter.query), reverse=True)
    ...
    def add_review(self, service_id: str, user_id: str, user_name: str, 
                  rating: int, title: str, content: str) -> Optional[Review]:
        ...
        existing_review = next(
            (r for r in self.reviews[service_id] if r.user_id == user_id), 
            None
        )
    ...
    def record_service_usage(self, service_id: str, user_id: str, 
                           call_cost: float = 0.0) -> Optional[ServiceUsage]:
        ...
        if user_id not in self.usage[service_id]:
            self.usage[service_id][user_id] = ServiceUsage(
                service_id=service_id,
                user_id=user_id
            )
    ...
    def get_popular_services(self, limit: int = 10, category: Optional[ServiceCategory] = None) -> List[Service]:
    ...
    def get_service_statistics(self) -> Dict[str, Any]:
        ...
        published_services = len([s for s in self.services.values() if s.status == ServiceStatus.PUBLISHED])
