...
from .models import Proposal, Vote, ProposalStatus, ProposalCategory, VoteType
...
class GovernanceManager:
    ...
    def cast_vote(
        ...
        existing_vote = self.db.query(Vote).filter(
            and_(
                ...
                Vote.voter_wallet_id == voter_wallet_id
            )
        ).first()
        
        if existing_vote:
            raise AlreadyVotedError(f"Wallet {voter_wallet_id} has already voted on proposal {proposal_id}")
    ...
    def get_vote_results(self, proposal_id: str) -> Dict[str, Any]:
        ...
        abstain_percentage = (abstain_power / total_voting_power * 100) if total_voting_power > 0 else 0
        ...
        # Determine outcome
        outcome = "pending"
        if proposal.has_expired() or proposal.status in [ProposalStatus.PASSED, ProposalStatus.REJECTED]:
    ...
    def get_governance_stats(self) -> Dict[str, Any]:
        ...
        passed_proposals = self.db.query(Proposal).filter(
            Proposal.status == ProposalStatus.PASSED
        ).count()
        executed_proposals = self.db.query(Proposal).filter(
            Proposal.status == ProposalStatus.EXECUTED
        ).count()
