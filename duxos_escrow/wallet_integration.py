...
try:
    from duxos_registry.services.wallet_service import FlopcoinWalletService, WalletService
    from duxos_registry.services.auth_service import NodeAuthService, AuthLevel
    from duxos_registry.db.repository import WalletRepository, TransactionRepository
    from duxos_registry.models.database_models import Wallet, Transaction
except ImportError:
...
from .models import Escrow, EscrowTransaction, CommunityFund
from .exceptions import WalletIntegrationError, InsufficientFundsError, TransactionFailedError
...
class EscrowWalletIntegration:
    ...
    def lock_funds(self, wallet_id: int, amount: float, escrow_id: str, 
    ...
    def transfer_funds(self, from_wallet_id: Optional[int], to_wallet_id: int, 
                      amount: float, escrow_id: str, 
                      auth_data: Optional[Dict[str, Any]] = None) -> str:
        """
        ...
        try:
            ...
            to_wallet = self.wallet_repo.get_wallet_by_id(to_wallet_id) if self.wallet_repo else None
            if not to_wallet:
    ...
    def _transfer_from_escrow(self, to_wallet_id: int, amount: float, escrow_id: str) -> str:
        ...
        try:
            ...
            if lock_info['amount'] < amount:
                raise InsufficientFundsError(f"Insufficient locked funds: {lock_info['amount']} < {amount}")
            ...
            transaction_info = self.flopcoin_service.send_transaction(
                ...
                amount=amount,
                comment=f"Escrow {escrow_id} payment"
            )
            ...
            logger.info(f"Transferred {amount} FLOP from escrow {escrow_id} to wallet {to_wallet_id}")
            return txid
    ...
    def _transfer_between_wallets(self, from_wallet_id: int, to_wallet_id: int, 
                                 amount: float, escrow_id: str) -> str:
        """Transfer funds between two wallets"""
        try:
            # Get wallet addresses
            from_wallet = self.wallet_repo.get_wallet_by_id(from_wallet_id) if self.wallet_repo else None
            to_wallet = self.wallet_repo.get_wallet_by_id(to_wallet_id) if self.wallet_repo else None
            ...
            transaction_info = self.flopcoin_service.send_transaction(
                to_address=to_wallet.address,
                amount=amount,
                comment=f"Escrow {escrow_id} transfer"
            )
            ...
            logger.info(f"Transferred {amount} FLOP from wallet {from_wallet_id} to wallet {to_wallet_id}")
            return txid
    ...
    def validate_transaction_signature(self, escrow: Escrow, signature: str, 
        ...
        try:
            if not self.auth_service:
                logger.warning("Authentication service not available, skipping signature validation")
                return True
    ...
    def _record_escrow_transaction(self, escrow_id: str, txid: str, amount: float, 
                                 to_wallet_id: Optional[int], transaction_type: str):
...
class EscrowTransactionSigner:
    ...
    def sign_escrow_creation(self, node_id: str, secret_key: str, escrow_data: Dict[str, Any]) -> str:
    ...
    def sign_escrow_release(self, node_id: str, secret_key: str, escrow_id: str, 
                           result_hash: str) -> str:
