...
class DisputeResolver:
    ...
    def create_dispute(self, 
                      ...
                      reason: str,
                      evidence: Optional[Dict[str, Any]] = None) -> Dispute:
    ...
    def resolve_dispute(self, 
                       ...
                       refund_amount: Optional[float] = None) -> bool:
    ...
    def get_disputes_by_wallet(self, wallet_id: int, status: Optional[DisputeStatus] = None) -> List[Dispute]:
    ...
    def get_dispute_statistics(self) -> Dict[str, Any]:
        ...
        resolved_disputes = self.db.query(Dispute).filter(Dispute.status == DisputeStatus.RESOLVED).count()
        rejected_disputes = self.db.query(Dispute).filter(Dispute.status == DisputeStatus.REJECTED).count()
        ...
        return {
            ...
            "resolution_rate": (resolved_disputes + rejected_disputes) / total_disputes if total_disputes > 0 else 0
        } 