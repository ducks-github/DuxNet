...
from .exceptions import AirdropError, CommunityFundError, InsufficientCommunityFundError
...
class CommunityFundManager:
    ...
    def __init__(
        self,
        db: Session,
        wallet_integration: EscrowWalletIntegration,
        ...
        self.airdrop_interval_hours = self.config.get(
    ...
    def get_fund_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            total_collected = (
                self.db.query(func.sum(EscrowTransaction.amount))
                .filter(EscrowTransaction.transaction_type == "community_fund")
                .scalar()
                or 0.0
            )
            ...
            return {
                ...
                "last_airdrop_at": (
                    fund.last_airdrop_at.isoformat() if fund.last_airdrop_at else None
                ),
    ...
    def _check_airdrop_trigger(self):
        ...
        try:
            ...
            if fund.balance >= fund.airdrop_threshold and (
                not fund.last_airdrop_at
                or datetime.now(timezone.utc) - fund.last_airdrop_at
                >= timedelta(hours=self.airdrop_interval_hours)
            ):

                logger.info(
                    f"Community fund threshold reached ({fund.balance} >= {fund.airdrop_threshold}), triggering airdrop"
                )
    ...
    def _trigger_airdrop(self):
        ...
        try:
            ...
            if airdrop_per_node < self.min_airdrop_amount:
                logger.warning(
                    f"Airdrop per node too small: {airdrop_per_node} < {self.min_airdrop_amount}"
                )
                return
            ...
            for node in active_nodes[: self.max_airdrop_nodes]:
                try:
                    ...
                    logger.info(
                        f"Airdropped {airdrop_per_node} FLOP to node {node['node_id']}, txid: {txid}"
                    )
            ...
            logger.info(
                f"Airdrop completed: {successful_airdrops} nodes, {total_distributed} FLOP distributed"
            )
    ...
    def manual_airdrop(self, node_ids: List[str], amount_per_node: float) -> Dict[str, Any]:
        ...
        try:
            ...
            if fund.balance < total_amount:
                raise InsufficientCommunityFundError(
                    f"Insufficient funds: {fund.balance} < {total_amount}"
                )
