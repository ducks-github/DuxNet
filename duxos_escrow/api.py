...
from .models import EscrowStatus, DisputeStatus
...
class CreateEscrowRequest(BaseModel):
    ...
    amount: float = Field(..., gt=0, description="Escrow amount in FLOP")
...
# Escrow endpoints
@app.post("/escrow/create", response_model=CreateEscrowResponse, dependencies=[Depends(rate_limiter), Depends(api_key_auth)])
...
@app.post("/escrow/{escrow_id}/release", dependencies=[Depends(rate_limiter), Depends(api_key_auth)])
...
@app.get("/escrow/wallet/{wallet_id}", response_model=List[EscrowInfo], dependencies=[Depends(rate_limiter)])
...
# Dispute endpoints
@app.post("/escrow/{escrow_id}/dispute", dependencies=[Depends(rate_limiter), Depends(api_key_auth)])
...
@app.post("/dispute/{dispute_id}/evidence", dependencies=[Depends(rate_limiter), Depends(api_key_auth)])
...
@app.post("/dispute/{dispute_id}/resolve", dependencies=[Depends(rate_limiter), Depends(api_key_auth)])
...
async def get_escrow_statistics(
    manager: EscrowManager = Depends(get_escrow_manager)
):
