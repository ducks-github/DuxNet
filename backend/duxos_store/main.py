...
def demo_mode(store_service: StoreService):
    ...
    for service_data in sample_services:
        try:
            service = store_service.register_service(
                service_data, service_data["owner_id"], service_data["owner_name"]
            )
    ...
    sample_reviews = [
        (
            ...
            5,
            "Excellent service",
            "Very accurate image recognition",
        ),
        (
            "Image Recognition API",
            "user2",
        ...
        (
            ...
            5,
            "Amazing",
            "Creates stunning visualizations",
        ),
    ]
        ...
        if services:
            ...
            try:
                store_service.add_review(
                    service.service_id, user_id, user_name, rating, title, content
                )
...
def main():
    ...
    print(
        f"API will be available at: http://{config.get('api', {}).get('host', '0.0.0.0')}:{config.get('api', {}).get('port', 8000)}"
    )
