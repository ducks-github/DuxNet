...
class NodeRepository:
    ...
    def create_node(
        ...
        """Create a new node"""
        node = Node(node_id=node_id, address=address)
        if metadata:
    ...
    def get_nodes_by_capability(self, capability_name: str, active_only: bool = True) -> List[Node]:
        ...
        query = (
            self.db.query(Node).join(Node.capabilities).filter(Capability.name == capability_name)
        )
        if active_only:
            query = query.filter(Node.is_active == True)
        return query.all()
    ...
    def add_capability_to_node(self, node_id: str, capability_name: str) -> Optional[Node]:
        ...
        if node:
            capability = (
                self.db.query(Capability).filter(Capability.name == capability_name).first()
            )
            if not capability:
...
class CapabilityRepository:
    ...
    def create_capability(
        self, name: str, description: Optional[str] = None, version: str = "1.0"
    ) -> Capability:
        """Create a new capability"""
        capability = Capability(name=name, description=description, version=version)
        self.db.add(capability)
        self.db.commit()
    ...
    def update_capability(
        self, name: str, description: Optional[str] = None, version: Optional[str] = None
    ) -> Optional[Capability]:
        """Update a capability"""
...
class ReputationRepository:
    ...
    def get_reputation_events(self, node_id: str, limit: int = 100) -> List[ReputationEvent]:
        ...
        return (
            ...
            .filter(ReputationEvent.node_id == node_id)
    ...
    def get_reputation_summary(self, node_id: str) -> Dict[str, Any]:
        ...
        return {"total_score": total_score, "event_count": event_count, "events": events}
...
class TransactionRepository:
    ...
    def get_transactions_by_node_since(self, node_id: str, since: datetime) -> List[Transaction]:
        ...
        return (
            self.db.query(Transaction)
            .join(Wallet)
            .filter(Wallet.node_id == node_id, Transaction.created_at >= since)
            .all()
        )
    ...
    def get_transaction_statistics(self, wallet_id: int) -> Dict[str, Any]:
        ...
        total_sent = sum(
            tx.amount for tx in transactions if getattr(tx, "transaction_type") == "send"
        )
        total_received = sum(
            tx.amount for tx in transactions if getattr(tx, "transaction_type") == "receive"
        )
