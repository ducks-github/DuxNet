...
from ..db.repository import TransactionRepository, WalletRepository
from ..models.database_models import Transaction, Wallet, WalletCapability
...
class FlopcoinWalletService:
    ...
    def __init__(
        ...
        rpc_port: int = 32553,
        rpc_user: str = "flopcoinrpc",
        rpc_password: str = "password123",
    ):
    ...
    def _make_rpc_call(self, method: str, params: Optional[List] = None) -> Dict:
        ...
        try:
            response = requests.post(self.rpc_url, json=payload, auth=self.auth, timeout=30)
    ...
    def send_transaction(
        self, to_address: str, amount: float, fee: Optional[float] = None, comment: str = ""
    ) -> Dict:
        ...
        try:
            ...
            return {
                ...
                "status": (
                    "pending"
                    if tx_info.get("confirmations", 0) < self.MIN_CONFIRMATIONS
                    else "confirmed"
                ),
            }
        except Exception as e:
...
class WalletService:
    ...
    def __init__(self, db_session: Session, config_path: Optional[str] = None):
        ...
        if config_path is None:
            config_path = os.path.join(
                os.path.dirname(__file__), "..", "..", "duxos", "wallet", "config.yaml"
            )
        ...
        # Security settings
        self.max_transaction_amount = self.config.get("wallet", {}).get(
            "max_transaction_amount", 1000.0
        )
        self.rate_limit_window = self.config.get("wallet", {}).get(
            "rate_limit_window", 3600
        )  # 1 hour
        self.max_transactions_per_window = self.config.get("wallet", {}).get(
    ...
    def create_wallet(
        self, node_id: str, wallet_name: str, auth_data: Optional[Dict] = None
    ) -> Dict:
    ...
    def send_transaction(
        self, node_id: str, recipient_address: str, amount: float, auth_data: Optional[Dict] = None
    ) -> Dict:
        """Send Flopcoin transaction"""
        try:
            # Authenticate node if required
            ...
            if amount > self.max_transaction_amount:
                return {
                    "success": False,
                    "message": f"Amount exceeds maximum limit of {self.max_transaction_amount}",
                }
    ...
    def get_transaction_history(
        self, node_id: str, limit: int = 50, auth_data: Optional[Dict] = None
    ) -> Dict:
