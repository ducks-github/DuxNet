...
from duxos_registry.services.auth_service import (
    AuthLevel,
    NodeAuthService,
    NodeIdentity,
)
...
class TestNodeAuthService:
    ...
    def test_generate_node_identity(self):
        ...
        secret_key, node_id = self.auth_service.generate_node_identity(
            self.test_node_id, AuthLevel.SIGNED
        )
    ...
    def test_create_and_verify_signature(self):
        ...
        signature = self.auth_service.create_signed_message(
            self.test_node_id, secret_key, self.test_message
        )
        assert signature is not None

        # Verify signature
        is_valid = self.auth_service.verify_node_signature(
            self.test_node_id, self.test_message, signature
        )
        ...
        is_valid = self.auth_service.verify_node_signature(
            self.test_node_id, self.test_message, invalid_signature
        )
    ...
    def test_authenticate_node(self):
        ...
        # Authenticate
        authenticated, auth_level = self.auth_service.authenticate_node(
            self.test_node_id, auth_data
        )
    ...
    def test_authorize_operation(self):
        ...
        assert (
            self.auth_service.authorize_operation(self.test_node_id, "register", AuthLevel.SIGNED)
            is True
        )
        assert (
            self.auth_service.authorize_operation(self.test_node_id, "update", AuthLevel.SIGNED)
            is True
        )
        assert (
            self.auth_service.authorize_operation(self.test_node_id, "query", AuthLevel.SIGNED)
            is True
        )

        # Test unauthorized operations
        assert (
            self.auth_service.authorize_operation(self.test_node_id, "admin", AuthLevel.SIGNED)
            is False
        )

        # Test with insufficient auth level
        assert (
            self.auth_service.authorize_operation(self.test_node_id, "register", AuthLevel.BASIC)
            is False
        )
