...
class Escrow(Base):
    ...
    provider_wallet_id: Mapped[int] = mapped_column(
        Integer, ForeignKey("wallets.id"), nullable=False
    )
    amount: Mapped[float] = mapped_column(Float, nullable=False)
    currency: Mapped[str] = mapped_column(String(10), nullable=False, default="FLOP")  # NEW FIELD
    status: Mapped[EscrowStatus] = mapped_column(
        Enum(EscrowStatus), default=EscrowStatus.PENDING, nullable=False
    )
    ...
    )  # Hash of expected result
    provider_signature: Mapped[Optional[str]] = mapped_column(
        String, nullable=True
    )  # Provider's signature
    payer_signature: Mapped[Optional[str]] = mapped_column(
        String, nullable=True
    )  # Payer's signature

    # Financial tracking
    provider_amount: Mapped[Optional[float]] = mapped_column(
        Float, nullable=True
    )  # Amount to provider (95%)
    community_amount: Mapped[Optional[float]] = mapped_column(
        Float, nullable=True
    )  # Amount to community (5%)
    ...
    # Metadata
    escrow_metadata: Mapped[str] = mapped_column(
        Text, default="{}"
...
class Dispute(Base):
    ...
    # Parties
    initiator_wallet_id: Mapped[int] = mapped_column(
        Integer, ForeignKey("wallets.id"), nullable=False
    )
    respondent_wallet_id: Mapped[int] = mapped_column(
        Integer, ForeignKey("wallets.id"), nullable=False
    )
...
class CommunityFund(Base):
    ...
    airdrop_threshold: Mapped[float] = mapped_column(
        Float, default=100.0, nullable=False
    )  # Trigger airdrop at 100 FLOP
    last_airdrop_at: Mapped[Optional[datetime]] = mapped_column(
    ...
    min_vote_threshold: Mapped[float] = mapped_column(
...
class EscrowTransaction(Base):
    ...
    amount: Mapped[float] = mapped_column(Float, nullable=False)
    currency: Mapped[str] = mapped_column(String(10), nullable=False, default="FLOP")  # NEW FIELD
    from_wallet_id: Mapped[Optional[int]] = mapped_column(
        Integer, ForeignKey("wallets.id"), nullable=True
    )
    to_wallet_id: Mapped[Optional[int]] = mapped_column(
        Integer, ForeignKey("wallets.id"), nullable=True
    )

    # Blockchain transaction
    blockchain_txid: Mapped[Optional[str]] = mapped_column(
        String, nullable=True
    )  # Flopcoin transaction ID
    ...
    def get_metadata(self) -> Dict[str, Any]:
        ...
        try:
            metadata_str = (
                str(self.transaction_metadata) if self.transaction_metadata is not None else "{}"
            )
            return json.loads(metadata_str)
        except json.JSONDecodeError:
            return {}
...
class Proposal(Base):
    ...
    # Proposal details
    proposer_wallet_id: Mapped[int] = mapped_column(
        Integer, ForeignKey("wallets.id"), nullable=False
    )
    required_quorum: Mapped[float] = mapped_column(Float, nullable=False)  # Minimum votes required
    voting_period_days: Mapped[int] = mapped_column(
    ...
    estimated_cost: Mapped[Optional[float]] = mapped_column(
        Float, nullable=True
    )  # Estimated cost in FLOP
    funding_source: Mapped[Optional[str]] = mapped_column(
        String, nullable=True
    )  # community_fund, treasury, etc.

    # Execution details
    execution_data: Mapped[str] = mapped_column(
        Text, default="{}"
    )  # JSON string for execution parameters
    ...
    voting_started_at: Mapped[Optional[datetime]] = mapped_column(
        DateTime(timezone=True), nullable=True
    )
...
class Vote(Base):
    ...
    voting_power: Mapped[float] = mapped_column(
        Float, nullable=False
    )  # Amount of FLOP used for voting
...
class Wallet(Base):
    """Minimal Wallet model for foreign key constraints"""
