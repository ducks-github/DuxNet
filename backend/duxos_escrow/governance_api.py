...
from .models import ProposalCategory, ProposalStatus, VoteType
...
class ProposalCreateRequest(BaseModel):
    ...
    description: str = Field(
        ..., min_length=20, max_length=5000, description="Proposal description"
    )
...
async def create_proposal(
    request: ProposalCreateRequest, manager: GovernanceManager = Depends(get_governance_manager)
):
...
@router.get(
    "/proposals", response_model=List[ProposalResponse], dependencies=[Depends(rate_limiter)]
)
...
async def get_proposal(
    proposal_id: str, manager: GovernanceManager = Depends(get_governance_manager)
):
...
async def cast_vote(
    request: VoteRequest, manager: GovernanceManager = Depends(get_governance_manager)
):
...
@router.get(
    "/votes/history/{voter_wallet_id}",
    response_model=List[VoteResponse],
    dependencies=[Depends(rate_limiter)],
)
async def get_voter_history(
    voter_wallet_id: int, manager: GovernanceManager = Depends(get_governance_manager)
):
...
async def get_governance_stats(manager: GovernanceManager = Depends(get_governance_manager)):
