...
def execute_airdrop(
    ...
    try:
        if not 0.0 <= distribution_ratio <= 1.0:
            raise HTTPException(
                status_code=400, detail="Distribution ratio must be between 0.0 and 1.0"
            )
...
def update_airdrop_threshold(
    threshold: float, manager: CommunityFundManager = Depends(get_community_fund_manager)
) -> Dict[str, str]:
...
def withdraw_from_fund(
    amount: float, reason: str, manager: CommunityFundManager = Depends(get_community_fund_manager)
) -> Dict[str, str]:
