...
class NodeRegistry:
    ...
    def register_node(self, node: Node) -> str:
        ...
        if existing_node:
            self._logger.warning(
                f"Node with wallet {node.wallet_address} already exists. Updating."
            )
    ...
    def update_node_health(
        self, node_id: str, load_average: float, memory_usage: float, disk_usage: float
    ) -> bool:
        """
    ...
    def list_nodes(
        ...
        min_capabilities: Optional[NodeCapabilities] = None,
    ) -> List[Node]:
        """
        ...
        filtered_nodes = [
            ...
            for node in self._nodes.values()
            if (
                node.reputation_score >= min_reputation
                and (not only_healthy or node.health.is_healthy)
                and (not min_capabilities or self._meets_capabilities(node, min_capabilities))
            )
        ]
    ...
    def _meets_capabilities(self, node: Node, min_capabilities: NodeCapabilities) -> bool:
        ...
        """
        return (
            node.capabilities.cpu_cores >= min_capabilities.cpu_cores
            and node.capabilities.memory_gb >= min_capabilities.memory_gb
            and node.capabilities.storage_gb >= min_capabilities.storage_gb
            and (not min_capabilities.gpu_enabled or node.capabilities.gpu_enabled)
        )
    ...
    def _load_nodes(self):
        ...
        try:
            ...
            self._nodes = {
                node_id: Node.from_dict(node_data) for node_id, node_data in nodes_data.items()
            }
        except Exception as e:
