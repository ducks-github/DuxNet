...
REQUEST_COUNT = prometheus_client.Counter(
    "duxnet_requests_total", "Total requests", ["method", "endpoint"]
)
...
class Monitoring:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        # Custom metrics
        self.daemon_uptime = prometheus_client.Gauge(
            "duxnet_daemon_uptime_seconds", "Daemon uptime in seconds"
        )
        self.config_reloads = prometheus_client.Counter(
            "duxnet_config_reloads_total", "Total config reloads"
        )
        self.message_queue_operations = prometheus_client.Counter(
            "duxnet_mq_operations_total", "Message queue operations", ["operation", "status"]
        )
...
class ServiceDiscoveryAgent:
    ...
    def _broadcast_presence(self):
        logging.info(
            f"Service Discovery: Broadcasting presence every {self.broadcast_interval} seconds."
        )
        ...
        message = {
            ...
            "address": {"ip": "0.0.0.0", "duxnet_port": self.config.get("duxnet_port")},
            "timestamp": time.time(),
        }
    ...
    def _listen_for_broadcasts(self):
        ...
        while self.running:
            try:
                ...
                if (
                    message.get("type") == "duxnet_discovery"
                    and message.get("daemon_id") != self.daemon_id
                ):
                    ...
                    if peer_duxnet_port:
                        ...
                        logging.info(
                            f"Service Discovery: Discovered peer: {peer_id} at {peer_address} (Type: {message.get('service_type')})"
                        )
    ...
    def __init__(self, config):
        ...
        def get_status():
            ...
            try:
                ...
                self.monitoring.record_request(
                    "GET", "/api/v1/status", time.time() - start_time, True
                )
                return jsonify(response)
            except Exception as e:
                self.monitoring.record_request(
                    "GET", "/api/v1/status", time.time() - start_time, False
                )
        ...
        def get_daemon_config():
            ...
            try:
                ...
                self.monitoring.record_request(
                    "GET", "/api/v1/config", time.time() - start_time, True
                )
                return jsonify(safe_config)
            except Exception as e:
                self.monitoring.record_request(
                    "GET", "/api/v1/config", time.time() - start_time, False
                )
                return jsonify({"error": str(e)}), 500
        ...
        def get_known_peers():
            ...
            active_peers = [
                peer
                for peer in self.service_discovery_agent.known_peers.values()
...
class DuxNetDaemon(DuxOSDaemon):
    def __init__(self, config):
        ...
        if self.tls_enabled:
            if not self.cert_path or not self.key_path:
                logging.error(
                    "TLS is enabled but cert_path or key_path is not specified in config.yaml"
                )
    ...
    def run(self):
        logging.info(
            f"DuxNetDaemon starting, listening on port {self.listen_port}"
        ...
        if self.health_check_enabled:
            try:
                ...
                logging.info(
                    f"Health check endpoint enabled on port {self.health_check_port}/healthz"
                )
            except Exception as e:
