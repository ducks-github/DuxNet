...
from .schemas import (
    NodeRegisterRequest, NodeRegisterResponse, NodeListResponse, NodeInfo, 
    ReputationUpdateRequest, ReputationUpdateResponse,
...
def add_node_capabilities(request: CapabilityAddRequest):
    ...
    try:
        result = registry_service.add_node_capabilities(
            request.node_id, 
            request.new_capabilities
        )
        return result
...
def remove_node_capabilities(request: CapabilityRemoveRequest):
    ...
    try:
        result = registry_service.remove_node_capabilities(
            request.node_id, 
            request.capabilities_to_remove
        )
        return result
    except ValueError as e:
        raise HTTPException(status_code=404, detail=str(e))
...
def query_nodes_by_capabilities(request: CapabilityQueryRequest):
    ...
    nodes = registry_service.get_nodes_by_capabilities(
        request.capabilities, 
        request.match_all
    )
    return NodeListResponse(nodes=[NodeInfo(**node) for node in nodes])
