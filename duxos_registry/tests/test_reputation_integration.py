...
class TestReputationIntegration:
    ...
    def test_complete_reputation_workflow(self):
        ...
        register_result = self.registry_service.register_node(
            "integration_test_node", "192.168.1.200:8000", ["compute", "storage", "gpu"]
        )
        ...
        result1 = self.reputation_service.update_reputation(
            "integration_test_node", ReputationEventType.TASK_SUCCESS
        )
        ...
        result2 = self.reputation_service.update_reputation(
            "integration_test_node", ReputationEventType.HEALTH_MILESTONE
        )
        ...
        result3 = self.reputation_service.update_reputation(
            "integration_test_node", ReputationEventType.TASK_FAILURE
        )
    ...
    def test_reputation_clamping_scenarios(self):
        ...
        # Test upper clamping
        self.reputation_service.update_reputation(
            "clamp_test_node", ReputationEventType.TASK_SUCCESS, custom_delta=95.0
        )
        result1 = self.reputation_service.update_reputation(
            "clamp_test_node", ReputationEventType.TASK_SUCCESS, custom_delta=10.0
        )
        ...
        self.reputation_service.update_reputation(
            "clamp_test_node", ReputationEventType.TASK_SUCCESS, custom_delta=50.0
        )
        result2 = self.reputation_service.update_reputation(
            "clamp_test_node", ReputationEventType.MALICIOUS_BEHAVIOR, custom_delta=-200.0
        )
    ...
    def test_custom_reputation_rules(self):
        ...
        result = self.reputation_service.update_reputation(
            "custom_rule_node", ReputationEventType.TASK_SUCCESS
        )
    ...
    def test_multiple_nodes_reputation(self):
        ...
        for i, node_id in enumerate(nodes):
            self.registry_service.register_node(node_id, f"192.168.1.{210+i}:8000", ["compute"])
