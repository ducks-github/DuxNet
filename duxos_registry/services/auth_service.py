...
class NodeAuthService:
    ...
    def authorize_operation(self, node_id: str, operation: str, auth_level: AuthLevel) -> bool:
        ...
        try:
            ...
            if auth_level.value < self.min_auth_level.value:
                logger.warning(f"Insufficient auth level for node {node_id}: {auth_level.value} < {self.min_auth_level.value}")
                return False
    ...
    def get_auth_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "rate_limited_nodes": len([n for n, attempts in self.auth_attempts.items() 
