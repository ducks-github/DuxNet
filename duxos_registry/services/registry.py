...
class NodeRegistryService:
    def __init__(self) -> None:
        ...
        self.capability_index: Dict[str, Set[str]] = {}  # capability -> set of node_ids
        # Standard capabilities that are pre-defined
        self.standard_capabilities: Set[str] = {cap.value for cap in CapabilityType}

    def register_node(self, node_id: str, address: str, capabilities: List[str]) -> Dict[str, Any]:
        """Register a new node with its capabilities."""
        # Validate capabilities
        validated_capabilities: List[str] = self._validate_capabilities(capabilities)
    ...
    def add_node_capabilities(self, node_id: str, new_capabilities: List[str]) -> Dict[str, Any]:
        """
        ...
        validated_new_capabilities: List[str] = self._validate_capabilities(new_capabilities)

        # Get current capabilities
        current_capabilities: Set[str] = set(self.nodes[node_id]["capabilities"])
        old_capabilities: List[str] = list(current_capabilities)

        # Add new capabilities (avoid duplicates)
        current_capabilities.update(validated_new_capabilities)
        updated_capabilities: List[str] = list(current_capabilities)
    ...
    def remove_node_capabilities(
        self, node_id: str, capabilities_to_remove: List[str]
    ) -> Dict[str, Any]:
        """
    ...
    def update_node_capabilities(self, node_id: str, new_capabilities: List[str]) -> Dict[str, Any]:
        """
        ...
        # Get current capabilities
        old_capabilities: List[str] = list(self.nodes[node_id]["capabilities"])
    ...
    def get_nodes_by_capability(self, capability: str) -> List[Dict[str, Any]]:
        """
        ...
        node_ids: Set[str] = self.capability_index[capability]
        return [self.nodes[node_id] for node_id in node_ids if node_id in self.nodes]

    def get_nodes_by_capabilities(
        ...
        matching_node_ids: Optional[Set[str]]
        if match_all:
            ...
            for capability in capabilities:
                ...
                if matching_node_ids is None:
                    matching_node_ids = set(node_ids)
                else:
    ...
    def get_capability_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "custom_capabilities": [
                cap for cap in capability_counts.keys() if cap not in self.standard_capabilities
            ],
        }
    ...
    def _update_capability_index(self, node_id: str, capabilities: List[str]) -> None:
        """
    ...
    def update_node_reputation(self, node_id: str, delta: float) -> Dict[str, Any]:
        """
        ...
        # Update reputation
        current_reputation: float = self.nodes[node_id]["reputation"]
        new_reputation: float = current_reputation + delta

        # Clamp reputation to reasonable bounds (0-100)
        clamped_reputation: float = max(0.0, min(100.0, new_reputation))
