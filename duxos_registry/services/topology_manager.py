...
class TopologyManager:
    ...
    def add_node(self, node_id: str, capabilities: List[str] = None, 
            ...
            with self.lock:
                ...
                if node_id in self.network_graph:
                    ...
                    connections_to_remove = [
                        (src, dst) for (src, dst) in self.connections.keys()
                    ...
                    routing_keys_to_remove = [
                        (src, dst) for (src, dst) in self.routing_cache.keys()
                        if src == node_id or dst == node_id
                    ]
    ...
    def add_connection(self, source_node: str, target_node: str, 
                      latency_ms: float, bandwidth_mbps: float, 
                      packet_loss: float = 0.0) -> bool:
        """Add connection between nodes"""
        try:
            with self.lock:
                # Determine connection quality
                quality = self._calculate_connection_quality(latency_ms, bandwidth_mbps, packet_loss)
                ...
                logger.debug(f"Added connection {source_node} -> {target_node} (latency: {latency_ms}ms)")
                return True
        except Exception as e:
    ...
    def create_segment(self, segment_id: str, name: str, description: str = "",
                      nodes: List[str] = None, gateway_nodes: List[str] = None,
                      bandwidth_capacity: float = 1000.0, 
                      latency_threshold: float = 50.0) -> bool:
    ...
    def find_optimal_path(self, source: str, destination: str, 
                         optimize_for: str = "latency") -> Optional[RoutingPath]:
        ...
        try:
            with self.lock:
                ...
                if not self.network_graph.has_node(source) or not self.network_graph.has_node(destination):
                    logger.warning(f"Source or destination node not found: {source} -> {destination}")
                    return None
                
                # Find shortest path based on optimization criteria
                if optimize_for == "latency":
                    path = nx.shortest_path(self.network_graph, source, destination, weight="latency")
                ...
                for i in range(len(path) - 1):
                    ...
                    if self.network_graph.has_edge(u, v):
                        ...
                        total_bandwidth = min(total_bandwidth, edge_data.get("bandwidth", float('inf')))
        ...
        try:
            with self.lock:
                ...
                return {
                    ...
                    "connections": {f"{src}->{dst}": conn.to_dict() 
    ...
    def optimize_topology(self) -> Dict[str, Any]:
        ...
        try:
            with self.lock:
                ...
                poor_connections = [
                    (src, dst) for (src, dst), conn in self.connections.items()
                ...
                old_cache_entries = [
                    key for key, path in self.routing_cache.items()
                ...
                optimization_results["performance_improvements"] = {
                    "latency_change": self.topology_stats["average_latency"] - old_stats["average_latency"],
                    "bandwidth_change": self.topology_stats["average_bandwidth"] - old_stats["average_bandwidth"],
                    "connectivity_change": self.topology_stats["connectivity_score"] - old_stats["connectivity_score"]
                }
    ...
    def _calculate_connection_quality(self, latency_ms: float, bandwidth_mbps: float, 
                                    packet_loss: float) -> ConnectionQuality:
    ...
    def _split_segment(self, segment_id: str):
        ...
        node_groups = [segment.nodes[i:i + self.max_segment_size] 
