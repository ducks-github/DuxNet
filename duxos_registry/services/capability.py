...
class NodeCapabilityService:
    ...
    def add_capability(self, node_id: str, capability: str) -> Dict:
        """
        ...
        node = self.registry_service.nodes[node_id]
        current_capabilities = set(node["capabilities"])
    ...
    def remove_capability(self, node_id: str, capability: str) -> Dict:
        """
    ...
    def update_capabilities(self, node_id: str, capabilities: List[str]) -> Dict:
        """
        ...
        node = self.registry_service.nodes[node_id]
        old_capabilities = node["capabilities"]
        
        # Update capabilities (remove duplicates)
        new_capabilities = list(set(capabilities))
    ...
    def get_nodes_with_capability(self, capability: str) -> List[Dict]:
        ...
        nodes = self.registry_service.get_nodes()
        return [node for node in nodes if capability in node["capabilities"]]
    
    def get_nodes_with_capabilities(self, capabilities: List[str], require_all: bool = True) -> List[Dict]:
        """
        ...
        if require_all:
            ...
            return [node for node in nodes if all(cap in node["capabilities"] for cap in capabilities)]
        else:
            # Return nodes that have ANY of the specified capabilities
            return [node for node in nodes if any(cap in node["capabilities"] for cap in capabilities)]
    ...
    def get_capability_statistics(self) -> Dict:
        ...
        for capability, count in capability_counts.items():
            capability_percentages[capability] = (count / total_nodes * 100) if total_nodes > 0 else 0
        
        return {
            "total_nodes": total_nodes,
            "capability_counts": capability_counts,
            "capability_percentages": capability_percentages,
            "most_common_capabilities": sorted(capability_counts.items(), key=lambda x: x[1], reverse=True)[:5]
        }
    ...
    def validate_capability(self, capability: str) -> Dict:
        ...
        """
        is_standard = capability in self.standard_capabilities
    ...
    def add_standard_capability(self, capability: str) -> Dict:
        """
    ...
    def get_node_capabilities(self, node_id: str) -> Dict:
        ...
        node = self.registry_service.get_node(node_id)
        if not node:
            return {
                "success": False,
                "error": f"Node {node_id} not found",
        ...
        standard_capabilities = [cap for cap in capabilities if cap in self.standard_capabilities]
        custom_capabilities = [cap for cap in capabilities if cap not in self.standard_capabilities]
