...
from .p2p_protocol import P2PProtocol, MessageType, P2PMessage
...
class IntegratedNodeRegistry:
    ...
    def __init__(self, node_id: str, db_session: Session, 
                 p2p_listen_port: int = 9334, p2p_broadcast_port: int = 9335):
            ...
            if existing_node:
                ...
                if self.auto_register_p2p_nodes:
                    ...
                    if self.require_authentication and not self.auth_service.get_node_identity(node_id):
                        secret_key, _ = self.auth_service.generate_node_identity(node_id, AuthLevel.SIGNED)
                    ...
                    result = self.db_registry.register_node(
                        ...
                        metadata={
                            ...
                            "auth_level": auth_level.value if self.require_authentication else "none"
                        }
                    )
        ...
        try:
            ...
            if node_id and address:
                ...
                if result["success"]:
                    ...
                    await self.p2p_protocol.broadcast_message(MessageType.NODE_UPDATE, {
                        "node_id": node_id,
                        "status": "registered",
                        "capabilities": capabilities
                    })
        ...
        while self.running:
            try:
                ...
                if current_node:
                    ...
                    await self.p2p_protocol.broadcast_message(MessageType.HEALTH_BROADCAST, health_data)
            ...
            try:
                ...
                for node in all_nodes:
                    ...
                    if last_heartbeat:
                        ...
                        try:
                            import datetime
                            last_seen = datetime.datetime.fromisoformat(last_heartbeat.replace('Z', '+00:00'))
                            ...
                            if current_time - last_seen_timestamp > 3600:
                                ...
                                logger.info(f"Marked node {node['node_id']} as offline due to inactivity")
                        except Exception as e:
                            logger.warning(f"Error parsing heartbeat for node {node['node_id']}: {e}")
    ...
    def register_node(self, node_id: str, address: str, 
        ...
        try:
            ...
            if result["success"]:
                ...
                asyncio.create_task(self.p2p_protocol.broadcast_message(MessageType.NODE_REGISTER, p2p_data))
    ...
    def update_node_status(self, node_id: str, status: str, auth_data: Optional[Dict] = None) -> Dict:
        ...
        try:
            ...
            if result["success"]:
                ...
                asyncio.create_task(self.p2p_protocol.broadcast_message(MessageType.NODE_UPDATE, p2p_data))
    ...
    def update_node_reputation(self, node_id: str, delta: float, auth_data: Optional[Dict] = None) -> Dict:
    ...
    def generate_node_identity(self, node_id: str, auth_level: AuthLevel = AuthLevel.SIGNED) -> Tuple[str, str]:
