...
from ..db.repository import CapabilityRepository, NodeRepository, ReputationRepository
...
class DatabaseNodeRegistryService:
        ...
        try:
            ...
            for capability_name in capabilities_to_remove:
                if capability_name in old_capabilities:
                    updated_node = self.node_repo.remove_capability_from_node(
                        node_id, capability_name
                    )
                    if updated_node:
        ...
        if match_all:
            ...
            if common_node_ids is not None:
                result = [
                    ...
                    if str(node.node_id) in common_node_ids
                ]
                return [node for node in result if node is not None]
            return []
        else:
            ...
            all_nodes = self.node_repo.get_all_nodes()
            result = [
                self._node_to_dict(node) for node in all_nodes if str(node.node_id) in any_node_ids
            ]
            return [node for node in result if node is not None]
    ...
    def _node_to_dict(self, node: Optional[Node]) -> Optional[Dict]:
        ...
        return {
            ...
            "last_heartbeat": (
                node.last_heartbeat.isoformat() if node.last_heartbeat is not None else None
            ),
