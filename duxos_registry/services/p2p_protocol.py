...
class P2PProtocol:
    ...
    async def stop(self):
        ...
        await self.broadcast_message(MessageType.GOODBYE, {
    ...
    async def broadcast_message(self, message_type: MessageType, payload: Optional[Dict[str, Any]] = None) -> None:
    ...
    async def send_message(self, target_address: str, message_type: MessageType, 
    ...
    async def _broadcast_presence(self):
        ...
        while self.running:
            try:
                await self.broadcast_message(MessageType.HELLO, {
                    "node_id": self.node_id,
                    "capabilities": [],
                    "reputation": 0.0,
    ...
    def get_network_stats(self) -> Dict[str, Any]:
        ...
        with self.lock:
            ...
            healthy_nodes = len([n for n in self.known_nodes.values() if n.health_status == "healthy"])
            avg_reputation = sum(n.reputation for n in self.known_nodes.values()) / total_nodes if total_nodes > 0 else 0
