...
REQUEST_COUNT = prometheus_client.Counter(
    "duxos_requests_total", "Total requests", ["method", "endpoint"]
)
...
class Monitoring:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.daemon_uptime = prometheus_client.Gauge(
            "duxos_daemon_uptime_seconds", "Daemon uptime in seconds"
        )
        self.config_reloads = prometheus_client.Counter(
            "duxos_config_reloads_total", "Total config reloads"
        )
        self.message_queue_operations = prometheus_client.Counter(
            "duxos_mq_operations_total", "Message queue operations", ["operation", "status"]
        )
    ...
    def _listen_for_broadcasts(self):
        ...
        while self.running:
            try:
                ...
                if (
                    message.get("type") == "duxos_discovery"
